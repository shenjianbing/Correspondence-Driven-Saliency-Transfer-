function [BSsaliency, mask]= centerBias(IniSal_region,IniSal,superpixels)
ratio = 1;
...
% bdCon = BoundaryConnectivity(ConSPix, colDistM, double(bdIds),clipVal, true);
% %bdCon = bdCon.*exp(1*salM);
% %bdCon = sum(bdCon)/sum(exp(IniSal_region));
% bdCon = max(bdCon);
% R = bdCon';
% R = (R-min(R))/(max(R)-min(R));
% BSsaliency = reshape(superpixel2pixel(double(Label),1-R),height,width);
...
R = (R-min(R))/(max(R)-min(R));
BSsaliency = R(Label);
BSsaliency = BSsaliency.*saliencycompact(BSsaliency,ratio);
R = getSuperpixelMeanValues(BSsaliency,Label,double(max(Label(:))));  
R = normalize(R);  
BSsaliency = R(Label);
...
mask = zeros(height,width);
   %  K-means Algorithm 
    ...
    for i=1:clus_num
        ...
        f_disxs = (superpixels.centres(bound_clus{i}(:),1) -f_x).^2;
